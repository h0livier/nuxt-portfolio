...
<template>
  <div class="hidden print:block bg-white px-4 py-3 text-[10.5px] leading-snug text-black">
    ...
    <cv-ats-section :title="$t('workExperience')">
      <article v-for="experience in props.experiences" :key="`${experience.name}-${experience.date}-${experience.title}`" class="mt-2.5 first:mt-0 break-inside-avoid">
        ...
        <div v-if="props.showMissions !== false && experience.missions?.length" class="mt-1.5">
          <p class="font-semibold">{{ pluralize($t('mission'), experience.missions.length) }}:</p>
          <div v-for="mission in experience.missions" :key="`${experience.name}-${mission.date}`" class="mt-1 pl-3">
            <div class="flex items-baseline justify-between gap-3">
              <p class="font-semibold">{{ mission.enterprise }} | {{ mission.title }}</p>
              <p class="text-right italic">{{ mission.date }}</p>
            </div>
            <p>{{ mission.description }}</p>
            <p v-if="mission.skills?.length" class="mt-0.5">
              <span class="font-semibold">{{ $t('skills') }}:</span>
              {{ mission.skills.join(', ') }}
            </p>
          </div>
    ...
    <cv-ats-section v-if="props.showCertifications !== false" :title="$t('certificationsTitle')">
      <article v-for="certification in props.certifications" :key="`${certification.name}-${certification.date}`" class="mt-1.5 first:mt-0 break-inside-avoid">
        <div class="flex items-baseline justify-between gap-3">
    ...
    <cv-ats-section :title="$t('education')">
      <article v-for="education in props.educations" :key="`${education.name}-${education.date}`" class="mt-1.5 first:mt-0 break-inside-avoid">
        <div class="flex items-baseline justify-between gap-3">
    ...
    <cv-ats-section :title="$t('skills')">
      <div v-for="skill in props.skills" :key="skill.name" class="mt-1 first:mt-0 break-inside-avoid">
        <p>
